...
educators.post("/sendMail", async (req, res) => {
  ...
  emailUser.map(async (id) => {
    ...
    const html = `<html> <head> <title></title> </head> <body style="font-family:Arial; margin: 15px;"><div> <img src="http://cdn.mcauto-images-production.sendgrid.net/9dce8bd65b7f8879/11bbd28e-de1d-4de6-adde-54f2812f64e6/902x902.png" width="45px" style="margin:auto; display:block; margin-top:20px;" /> <h1 style="font-size:20px; text-align:center;">OLL</h1> </div> <h6 style="font-size:15px; text-align:center;">Payslip for the month of ${
      month[new Date().getMonth() - 2]
    } ${new Date().getFullYear()}<span style="font-weight:500"></span></h6> <table style="padding:10px; width:100%; font-size:15px; border:1px solid black;"> <tr> <td>${
      e.name
    }</td> <td>${e.bankName}</td> </tr> <tr> <td>${e.email}</td> <td>${
      e.accountNo
    }</td> </tr> <tr> <td>${e.address}</td> <td>${
      e.ifscCode
    }</td> </tr> </div> <table style="width:100%; text-align:center; font-size:12.5px; margin-top:15px;" border=1> <tr style="color:white;"> <th style="background-color:red; padding:6px 0;">Earning</th> <th style="background-color:red; padding:6px 0;">Amount</th> <th style="background-color:gray; padding:6px 0;">Deduction</th> <th style="background-color:gray; padding:6px 0;">Amount</th> </tr> <tr style="color:black;"> <td style="padding:6px 0; font-weight:550;">Basic Pay</td> <td style="padding:6px 0;">Rs. ${
      e.basicPay
